...
Settings.ready()
  .then(() => {
    ...
    if (Settings.getValue('device-artwork') === false) {
      deviceList.hideArtwork();
    }

    if(Settings.getValue('zoom')) {
      zoomWidget.setValue(Settings.getValue('zoom'));
    }
...
function initOptions() {
  dropDownMenu.addItem('Device artwork', (state) => {
    ...
    Settings.setValue('device-artwork', state);
  }, !!Settings.getValue('device-artwork'));
