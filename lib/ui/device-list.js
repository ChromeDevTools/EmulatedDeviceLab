...
class DeviceList {
  constructor(rootElement) {
    ...
    var zoomInputElem = (this._rootElement).querySelector('nav input.zoom');
    zoomInputElem.setAttribute('min', SCALE_MIN);
    zoomInputElem.setAttribute('max', SCALE_MAX);
    zoomInputElem.setAttribute('step', SCALE_STEP);

    (this._rootElement).querySelector('nav .zoom-in').addEventListener('click', () => {
      deviceList.setScale(deviceList._scale + SCALE_STEP);
    });
    (this._rootElement).querySelector('nav .zoom-out').addEventListener('click', () => {
      deviceList.setScale(deviceList._scale - SCALE_STEP);
    });
    zoomInputElem.addEventListener('change', () => {
      deviceList.setScale(zoomInputElem.value);
    });
  ...
  setScale(scale) {
    scale = parseFloat(scale) || SCALE_MIN;
    ...
    (this._rootElement).querySelector('nav input.zoom').value = scale;
    (this._rootElement).querySelector('nav .zoom-value').innerText = scale.toFixed(1) + 'x';
  }
