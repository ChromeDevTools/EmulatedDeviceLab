...
class DeviceListItem {
  ...
  constructor(device) {
    ...
    /* DEVICE MENU SETUP */
    var deviceMenu = newItem.querySelector('.device-menu');
    var deviceMenuBtn = newItem.querySelector('.device-menu > i');
    var deviceShowWindowBtn = deviceMenu.querySelector('.device-show-window');
    var deviceCloseBtn = deviceMenu.querySelector('.device-close');

    deviceMenuBtn.addEventListener('click', () => {
      deviceMenu.classList.toggle('visible');
    });

    deviceShowWindowBtn.addEventListener('click', () => {
      device.bringToFront();
    ...
    function forwardMouseEvent(e) {
      if (e.target === deviceListItem._videoElement) {
        ...
        device.sendPointerEvent(e.type, x, y);
      }
    }
    ...
    function forwardScrollEvent(e) {
      if (e.target === deviceListItem._videoElement) {
        ...
        if (e.wheelDeltaX || e.wheelDeltaY) {
          device.sendScrollEvent(x, y, e.wheelDeltaY, e.wheelDeltaX);
        }
  ...
  _updateArtwork() {
    if(this._showArtwork) {
      ...
      if(deviceType && deviceType.artwork) {
